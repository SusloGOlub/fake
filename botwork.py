...
keyboard2.row('💵Сделать заказ💵', '🔥Качество🔥', '🚚Получение и оплата💴', '🔶Цены🔶', '🔴ВАЖНО🔴')
...
keyboard4.row('🇷🇺Рубли🇷🇺',  '🇺🇦Гривны🇺🇦', '🇱🇷Доллары🇱🇷', '⬅️ Назад')


@bot.message_handler(commands=['start'])
def start_message(message):
    bot.send_message(message.chat.id, 'Привет, ты попал к боту по продаже фальша😊💵', reply_markup=keyboard3)


@bot.message_handler(content_types=['text'])
def send_text(message):
    if message.text.lower() == 'привет! 👋':
        bot.send_message(message.chat.id, 'Ниже ты можешь более подробно узнать о наших купюрах.', reply_markup=keyboard2)
    	...
    	bot.send_message(message.chat.id, 'Возвращаемся назад', reply_markup=keyboard2)
